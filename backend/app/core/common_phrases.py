COMMON_METHOD_PHRASES = [
    "estudio observacional",
    "diseno transversal",
    "se realizo un analisis descriptivo",
    "se considero estadisticamente significativo",
    "intervalo de confianza del 95%",
]


ACADEMIC_REVIEW_WARNING = (
    "Este reporte identifica similitudes textuales y requiere interpretacion academica. "
    "No constituye una determinacion automatica de plagio."
)

SEMANTIC_WARNING = (
    "Las posibles parafrasis semanticas son senales de revision, no evidencia concluyente "
    "de mala practica academica."
)
